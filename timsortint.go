...
func Ints(a []int, lt IntLessThan) (err error) {
	lo := 0
	hi := len(a)
	nRemaining := hi
	...
	if nRemaining < minMerge {
		initRunLen, err := countRunAndMakeAscendingI(a, lo, hi, lt)
		if err != nil {
			return err
		}

		return binarySortI(a, lo, hi, lo+initRunLen, lt)
	}
	...
	for {
		// Identify next run
		runLen, err := countRunAndMakeAscendingI(a, lo, hi, lt)
		if err != nil {
			return err
		}
		...
		if runLen < minRun {
			...
			if err = binarySortI(a, lo, lo+force, lo+runLen, lt); err != nil {
				return err
			}
			runLen = force
		}
 ...
 */
func binarySortI(a []int, lo, hi, start int, lt IntLessThan) (err error) {
...
/**
  ...
  *          the specified array
*/
func countRunAndMakeAscendingI(a []int, lo, hi int, lt IntLessThan) (int, error) {
 ...
 */
func (hi *timSortHandlerI) mergeForceCollapse() (err error) {
	for hi.stackSize > 1 {
		n := hi.stackSize - 2
 ...
 */
func (hi *timSortHandlerI) mergeAt(i int) (err error) {
	if hi.stackSize < 2 {
	...
	if i != hi.stackSize-2 && i != hi.stackSize-3 {
	 ...
	 */
	k, err := gallopRightI(hi.a[base2], hi.a, base1, len1, 0, hi.lt)
	if err != nil {
		return err
	}
	if k < 0 {
	 ...
	 */
	len2, err = gallopLeftI(hi.a[base1+len1-1], hi.a, base2, len2, len2-1, hi.lt)
	if err != nil {
		return
	}
	if len2 < 0 {
	...
	if len1 <= len2 {
		err = hi.mergeLo(base1, len1, base2, len2)
		if err != nil {
			return fmt.Errorf("mergeLo: %v", err)
		}
	} else {
		err = hi.mergeHi(base1, len1, base2, len2)
		if err != nil {
 ...
 *    should follow it.
 */
func gallopLeftI(key int, a []int, base, len, hint int, c IntLessThan) (int, error) {
 ...
 * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
 */
func gallopRightI(key int, a []int, base, len, hint int, c IntLessThan) (int, error) {
	if len <= 0 || hint < 0 || hint >= len {
		return 0, errors.New(" len > 0 && hint >= 0 && hint < len;")
	}
 ...
 */
func (hi *timSortHandlerI) mergeLo(base1, len1, base2, len2 int) (err error) {
...
outer:
	for {
		...
		for {
			...
			count1, err = gallopRightI(a[cursor2], tmp, cursor1, len1, 0, lt)
			if err != nil {
				return
			}
			if count1 != 0 {
			...
			count2, err = gallopLeftI(tmp[cursor1], a, cursor2, len2, 0, lt)
			if err != nil {
				return
			}
			if count2 != 0 {
 ...
 * @param len2  length of second run to be merged (must be > 0)
 */
func (hi *timSortHandlerI) mergeHi(base1, len1, base2, len2 int) (err error) {
	if len1 <= 0 || len2 <= 0 || base1+len1 != base2 {
		return errors.New("len1 > 0 && len2 > 0 && base1 + len1 == base2;")
	}
...
outer:
	for {
		...
		for {
			...
			if gr, err := gallopRightI(tmp[cursor2], a, base1, len1, len1-1, lt); err == nil {
				count1 = len1 - gr
			} else {
				return err
			}
			...
			if gl, err := gallopLeftI(a[cursor1], tmp, 0, len2, len2-1, lt); err == nil {
				count2 = len2 - gl
			} else {
				return err
			}
